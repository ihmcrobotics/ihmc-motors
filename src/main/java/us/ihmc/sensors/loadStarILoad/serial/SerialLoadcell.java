...
public class SerialLoadcell
{
   ...
   public boolean connect()
   {
      Arrays.asList(SerialPortList.getPortNames())
            .stream()
            .filter(name -> ((!COMPort.isEmpty() && name.equals(COMPort)) || (COMPort.isEmpty() && USUAL_PORTS.stream().anyMatch(p -> name.startsWith(p)))))
            .findFirst()
            .ifPresent(name ->
                       {
                          try
                          {
                             ...
                             {
                                serialPort.setParams(SERIAL_BAUDRATE, SerialPort.DATABITS_8, SerialPort.STOPBITS_1, SerialPort.PARITY_NONE);
