...
public class Serial
{
   ...
   public boolean connect()
   {
      Arrays.asList(SerialPortList.getPortNames())
            .stream()
            .filter(name -> ((!arduinoPort.isEmpty() && name.equals(arduinoPort)) || (arduinoPort.isEmpty() && USUAL_PORTS.stream()
            ...
            .ifPresent(name ->
                       {
                          try
                          {
                             ...
                             {
                                serialPort.setParams(ARDUINO_SERIAL_BAUDRATE, SerialPort.DATABITS_8, SerialPort.STOPBITS_1, SerialPort.PARITY_NONE);
                             }
      ...
      serialPort.addEventListener(event ->
                                  {
                                     if (event.isRXCHAR())
                                     {
                                        try
                                        {
                                           ...
                                           {
                                              // add timestamp
                                              line.set(Long.toString(System.currentTimeMillis()).concat(SEPARATOR).concat(ch.substring(0, ch.indexOf("\r\n"))));
                                              stringBuilder = new StringBuilder();
                                           }
                                        }
                                        catch (SerialPortException e)
